...
export default async function SearchPage({searchParams}:{searchParams:Promise<{q?:string,page?:string}>}){
  ...
  return <main className={styles.main}><div className={styles.heading}><p>Catalog search</p><h1>{query?`Results for “${query}”`:"Find your next card"}</h1>{query&&<p>{result.total_results.toLocaleString()} cards found</p>}<SearchForm defaultValue={query}/></div><section className={styles.results} aria-label="Catalog search results">{result.cards.length?<ul className={styles.grid}>{result.cards.map(card=><li key={card.canonicalCardId}><CardTile card={card} layout="grid"/></li>)}</ul>:<p className={styles.empty}>{query?"No matching cards were found. Try another card name.":"Enter a card name to explore the catalog."}</p>}{result.total_pages>0&&<nav className={styles.pagination} aria-label="Search result pages"><p>Showing {start}–{end} of {result.total_results.toLocaleString()} · Page {result.page} of {result.total_pages}</p><div>{result.page>1?<Link href={href(result.page-1)}>Previous</Link>:<span/>}{result.page<result.total_pages&&<Link href={href(result.page+1)}>Next</Link>}</div></nav>}</section></main>
}
