...
const listingLabel = (cash: boolean, trade: boolean) => cash && trade ? "TRADE + SALE" : trade ? "TRADE" : "FOR SALE";
const formatPrice = (amount: string, currency: string) => {
  try { return new Intl.NumberFormat(undefined, { style: "currency", currency }).format(Number(amount)); }
  catch { return `${currency} ${amount}`; }
};

export function CardTile({ card, layout = "rail" }: { card: CardView; layout?: "rail" | "grid" }) {
  ...
  return (
    <article className={`${styles.tile} ${layout === "grid" ? styles.gridTile : ""}`}>
      ...
      <div className={styles.details}>
        ...
        {card.listing && <div className={styles.deal}><span className={card.listing.acceptsCash && card.listing.acceptsTrade ? styles.mixed : card.listing.acceptsTrade ? styles.trade : styles.cash}>{listingLabel(card.listing.acceptsCash, card.listing.acceptsTrade)}</span>{card.listing.acceptsCash && card.listing.askingPrice && card.listing.currencyCode && <strong>{formatPrice(card.listing.askingPrice, card.listing.currencyCode)}</strong>}</div>}
