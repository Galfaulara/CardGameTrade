...
import { InterestAction } from "../interest-action/interest-action";
  ...
  return (
    <article className={`${styles.tile} ${layout === "grid" ? styles.gridTile : ""}`}>
      ...
      <div className={styles.details}>
        ...
        {relationship&&(relationship.owned>0||relationship.inWants)?<div className={styles.tags}>{relationship.owned>0?<span>Owned ×{relationship.owned}</span>:null}{relationship.inWants?<span>In Wishlists</span>:null}</div>:null}
        ...
        {card.source?.kind === "user" ? <InterestAction inventoryItemId={card.id} cardName={card.name} imageUrl={card.imageUrl} /> : null}
      </div>
    </article>
  );
}
