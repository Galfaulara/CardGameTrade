    ...
    select: {
      card_printings: {
        select: {
          ...
          canonical_cards: {
            select: { id: true, name: true, mana_cost: true, type_line: true },
          },
...
export class DiscoveryService {
  ...
  private decodeCursor<T extends object>(
    ...
    try {
      ...
      if (
        ...
        value.kind !== kind ||
        value.filter !== filter ||
        (value.gameId ?? null) !== (gameId ?? null)
      )
        throw new Error();
      return value as T;
    } catch {
      throw new BadRequestException(
        "The discovery cursor is malformed or does not match these filters.",
      );
    }
  }
  ...
  private mapItem(item: any) {
    return {
      ...
      status: item.status,
      printing: item.printing_finishes.card_printings,
    };
  }
  ...
  private mapPreferredStore(store: any) {
    if (
      ...
      !store.trade_mediation_enabled
    )
      return null;
    const {
      status: _status,
      verification_status: _verification,
      trade_mediation_enabled: _mediation,
      ...safeStore
    } = store;
    return safeStore;
  }
  ...
  private async representativePrintings(canonicalIds: string[]) {
    ...
    return new Map(
      printings.map((printing) => [printing.canonical_card_id, printing]),
    );
  }
  ...
  async getUser(id: string) {
    ...
    const [
      ...
      activeListingCount,
      publicWishlistCount,
      publicWishlistItemCount,
    ] = await Promise.all([
      this.database.client.collections.count({
        where: { user_id: id, visibility: "public" },
      }),
      this.database.client.listings.count({ where: listingWhere }),
      this.database.client.wishlists.count({
        where: { user_id: id, visibility: "public", status: "active" },
      }),
      this.database.client.wishlist_items.count({
        where: {
          status: "active",
    ...
    return {
      user: {
        ...identity,
        preferred_store: this.mapPreferredStore(preferred_store),
      },
      summary: {
        public_collection_count: publicCollectionCount,
        active_listing_count: activeListingCount,
        public_wishlist_count: publicWishlistCount,
        public_wishlist_item_count: publicWishlistItemCount,
      },
    };
  }
  ...
  async getUserCollections(id: string, query: DiscoveryUserCollectionQuery) {
    ...
    const [allCounts, marketCounts, previews] = await Promise.all([
      this.database.client.inventory_items.groupBy({
        by: ["collection_id"],
        where: { collection_id: { in: ids }, owner_user_id: id, ...available },
        _sum: { quantity: true },
        _count: { _all: true },
      }),
      this.database.client.inventory_items.groupBy({
    ...
    const allById = new Map(
      allCounts.map((value: any) => [value.collection_id, value]),
    );
    const marketById = new Map(
      marketCounts.map((value: any) => [value.collection_id, value]),
    );
    return {
      user: {
        id: user.id,
      ...
      items: collections.map((collection) => ({
        ...
        marketplace_inventory_row_count:
          marketById.get(collection.id)?._count._all ?? 0,
        marketplace_card_quantity:
          marketById.get(collection.id)?._sum.quantity ?? 0,
  ...
  async getUserWishlists(id: string, query: DiscoveryUserWishlistQuery) {
    ...
    const [wishlists, total] = await Promise.all([
      this.database.client.wishlists.findMany({
        where,
        select: {
          id: true,
          game_id: true,
    ...
    const counts = await this.database.client.wishlist_items.groupBy({
      by: ["wishlist_id"],
      where: { wishlist_id: { in: ids }, status: "active" },
      _count: { _all: true },
      _sum: { quantity_desired: true },
    });
      ...
      if (values.length < query.previewLimit) {
        const printing =
          ...
          (item.canonical_card_id
            ...
            : null);
        const canonical =
          item.canonical_cards ?? item.card_printings?.canonical_cards ?? null;
        values.push({
          id: item.id,
          ...
          target: {
            ...
            printing: item.printing_id ? printing : null,
    ...
    const countsById = new Map(
      counts.map((value: any) => [value.wishlist_id, value]),
    );
    return {
      user: {
        id: user.id,
      ...
      items: wishlists.map((wishlist) => ({
        ...
        wanted_card_quantity:
          countsById.get(wishlist.id)?._sum.quantity_desired ?? 0,
  ...
  private async rankedPreviews(
    kind: "collection" | "store",
    ids: string[],
    limit: number,
  ) {
    ...
    const owner =
      kind === "collection"
        ? Prisma.raw("i.collection_id")
        : Prisma.raw("i.owner_store_id");
    const ownership =
      kind === "collection"
    ...
    const listings = await this.database.client.listings.findMany({
      where: { id: { in: ranked.map((row) => row.listing_id) } },
      select: previewSelect,
    });
    ...
    for (const row of ranked) {
      ...
      values.push(this.mapPreview(listing));
  ...
  async getCollectionFeed(query: DiscoveryCollectionFeedQuery) {
    ...
    const cursor = this.decodeCursor<Cursor>(
      query.cursor,
      "collections",
      query.availability,
      gameId,
    ...
    if (
      cursor &&
      (!cursor.id || !cursorDate || Number.isNaN(cursorDate.getTime()))
    ) {
      throw new BadRequestException(
        "The discovery cursor is malformed or does not match these filters.",
      );
    ...
    const where: Prisma.collectionsWhereInput = {
      ...
      ...(gameId ? { game_id: gameId } : {}),
      ...(query.availability === "marketplace"
        ? { inventory_items: { some: marketplace } }
        : {}),
      ...(cursor && cursorDate
        ? {
            OR: [
    ...
    const rows = await this.database.client.collections.findMany({
      where,
      select: {
        id: true,
        game_id: true,
    ...
    const collections = rows.slice(0, query.limit);
    const ids = collections.map((value) => value.id);
    const [allCounts, marketCounts, previews] = await Promise.all([
      this.database.client.inventory_items.groupBy({
        by: ["collection_id"],
        where: { collection_id: { in: ids }, ...available },
        _sum: { quantity: true },
        _count: { _all: true },
      }),
      this.database.client.inventory_items.groupBy({
        by: ["collection_id"],
        where: { collection_id: { in: ids }, ...marketplace },
        _sum: { quantity: true },
        _count: { _all: true },
      }),
      this.rankedPreviews("collection", ids, query.previewLimit),
    ...
    const items = collections.map(
      ({ user_profiles, user_id: _ownerId, ...metadata }) => {
        const { preferred_store, ...owner } = user_profiles;
        return {
          ...metadata,
          owner,
          preferred_store: this.mapPreferredStore(preferred_store),
          inventory_row_count: allById.get(metadata.id)?._count._all ?? 0,
          card_quantity: allById.get(metadata.id)?._sum.quantity ?? 0,
          marketplace_inventory_row_count:
            marketById.get(metadata.id)?._count._all ?? 0,
          marketplace_card_quantity:
            marketById.get(metadata.id)?._sum.quantity ?? 0,
  ...
  async getStoreFeed(query: DiscoveryStoreFeedQuery) {
    ...
    if (cursor && (typeof cursor.name !== "string" || !cursor.id))
      throw new BadRequestException(
        "The discovery cursor is malformed or does not match these filters.",
      );
    const marketplace = {
    ...
    const rows = await this.database.client.stores.findMany({
      where,
      select: {
        id: true,
        name: true,
    ...
    const hasMore = rows.length > query.limit;
    const stores = rows.slice(0, query.limit);
    const ids = stores.map((value) => value.id);
    const [allCounts, marketCounts, previews] = await Promise.all([
      this.database.client.inventory_items.groupBy({
        by: ["owner_store_id"],
        where: {
          owner_store_id: { in: ids },
          owner_user_id: null,
          ...available,
        },
        _sum: { quantity: true },
      ...
      this.database.client.inventory_items.groupBy({
        by: ["owner_store_id"],
        where: { owner_store_id: { in: ids }, ...marketplace },
        _sum: { quantity: true },
        _count: { _all: true },
      }),
    ...
    const allById = new Map(
      allCounts.map((value: any) => [value.owner_store_id, value]),
    );
    const marketById = new Map(
      marketCounts.map((value: any) => [value.owner_store_id, value]),
    );
    const items = stores.map((store) => ({
      ...store,
      inventory_row_count: allById.get(store.id)?._count._all ?? 0,
      card_quantity: allById.get(store.id)?._sum.quantity ?? 0,
      marketplace_inventory_row_count:
        marketById.get(store.id)?._count._all ?? 0,
      marketplace_card_quantity: marketById.get(store.id)?._sum.quantity ?? 0,
      preview_items: previews.get(store.id) ?? [],
    }));
  ...
  async getListingFeed(query: DiscoveryListingFeedQuery) {
    ...
    const intent =
      query.intent === "trade"
        ? { accepts_trade: true, accepts_cash: false }
        : query.intent === "sale"
          ? { accepts_trade: false, accepts_cash: true }
          : query.intent === "mixed"
          ...
          ? [
              {
                OR: [
                  ...
                  { created_at: cursorDate, id: { gt: cursor.id } },
                ],
              },
            ]
          : []),
    ...
    const rows = await this.database.client.listings.findMany({
      where,
      select: {
        ...previewSelect,
        seller_user_id: true,
    ...
    const hasMore = rows.length > query.limit;
    const listings = rows.slice(0, query.limit);
    const last = listings.at(-1);
    return {
      items: listings.map((listing) => this.mapFeedListing(listing)),
      next_cursor:
        hasMore && last
          ? this.encodeCursor({
              v: 1,
  ...
  async getCollections(query: DiscoveryCollectionListQuery) {
    ...
    const candidates = await this.database.client.collections.findMany({
      where: {
        ...
        user_profiles: { status: "active" },
        inventory_items: {
          some: query.marketplaceOnly ? marketplace : available,
        },
      },
      select: {
        id: true,
      ...
      orderBy: [{ updated_at: "desc" }, { id: "asc" }],
      take: query.diverseOwners ? query.limit * 4 : query.limit,
    });
    ...
    const candidateById = new Map(
      [...candidates, ...distinctOwnerCandidates].map((value) => [
        value.id,
        value,
      ]),
    );
    const rankedCandidates = [...candidateById.values()].sort(
      (a, b) =>
        b.updated_at.getTime() - a.updated_at.getTime() ||
        a.id.localeCompare(b.id),
    );
    const collections = query.diverseOwners
      ? this.diverse(rankedCandidates, query.limit)
      : candidates;
    const ids = collections.map((value) => value.id);
    const [allCounts, marketCounts, previews] = await Promise.all([
      this.database.client.inventory_items.groupBy({
  ...
  async getCollection(id: string, query: DiscoveryInventoryPageQuery) {
    ...
    const where = {
      collection_id: id,
      owner_user_id: collection.user_profiles.id,
      ...available,
    } as const;
    ...
    return {
      collection: {
        ...
        card_quantity: aggregate._sum.quantity ?? 0,
      },
  ...
  async getStores(query: DiscoveryStoreListQuery) {
    ...
    const stores = await this.database.client.stores.findMany({
      where: {
        ...eligibleStore,
        inventory_items: {
          some: query.marketplaceOnly
            ? marketplace
            : { ...available, owner_user_id: null },
        },
      },
      select: {
        id: true,
  ...
  async getStore(id: string) {
    ...
    const [inventory, marketplaceInventory, activeListingCount] =
      await Promise.all([
        this.database.client.inventory_items.aggregate({
