...
export class CatalogSyncService {
  ...
  private async getBulkDataItem(type: string) {
  ...
  private async ensureBulkDataFile(type: string) {
    ...
    const cacheDirectory = join(process.cwd(), ".cache", "scryfall");

    await mkdir(cacheDirectory, {
      recursive: true,
    });

    const finalPath = join(cacheDirectory, `${type}-${version}.jsonl.gz`);

    const partialPath = `${finalPath}.part`;

    const finalSize = await this.getFileSize(finalPath);

    if (finalSize === bulkData.compressed_size) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      if (downloadedBytes > bulkData.compressed_size) {
      ...
      const percentage =
        bulkData.compressed_size > 0
          ? ((downloadedBytes / bulkData.compressed_size) * 100).toFixed(1)
          : "0.0";
      ...
      if (downloadedSize === bulkData.compressed_size) {
      ...
      if (
        downloadedSize !== null &&
        downloadedSize > bulkData.compressed_size
      ) {
        await unlink(partialPath);
  ...
  private async *readBulkDataCards(type: string) {
    const filePath = await this.ensureBulkDataFile(type);

    this.logger.log(`Reading ${type} from local cache...`);

    const compressedInput = createReadStream(filePath);

    const decompressedInput = compressedInput.pipe(createGunzip());
  ...
  private getCardImageUris(card: ScryfallCard) {
    if (card.image_uris) {
  ...
  async syncSets() {
    ...
    for (const set of physicalSets) {
      await this.database.client.card_sets.upsert({
        ...
        create: {
          ...
          source: "scryfall",
          source_id: set.id,
          release_date: set.released_at ? new Date(set.released_at) : null,
    ...
    return {
      ...
      skippedDigital: result.data.length - physicalSets.length,
      synchronized,
    };
  }
  ...
  async syncCanonicalCards() {
    ...
    for await (const { rawData, card } of this.readBulkDataCards(
      "oracle_cards",
    )) {
      if (!card.oracle_id) {
      ...
      await this.database.client.canonical_cards.upsert({
        ...
        create: {
          ...
          source_key: card.oracle_id,
          name: card.name,
          normalized_name: normalizeCatalogSearchText(card.name),
  ...
  async syncPrintings() {
    ...
    const setIdByScryfallId = new Map(
      sets
        .filter(
          (
            set,
        ...
        .map((set) => [set.source_id, set.id]),
    );

    const canonicalIdByOracleId = new Map(
      canonicalCards.map((card) => [card.source_key, card.id]),
    );
    ...
    for await (const { rawData, card } of this.readBulkDataCards("all_cards")) {
      ...
      const printing = await this.database.client.card_printings.upsert({
        ...
        create: {
          ...
          image_small_uri: imageUris?.small ?? null,
          image_normal_uri: imageUris?.normal ?? null,
          image_large_uri: imageUris?.large ?? null,
      ...
      const finishes = [...new Set(card.finishes ?? [])].filter(
        (finish) => finish.trim().length > 0,
      );
      ...
      if (finishes.length > 0) {
        ...
        finishesSynchronized += result.count;
      }
