...
async function rejectsUnknown(action: () => Promise<unknown>) {
  try {
    await action();
  } catch (error) {
...
async function main() {
  ...
  try {
    const current = await database.client.$queryRaw<Array<{ current_database: string }>>`SELECT current_database()`;
    ...
    const offers = new OffersService(database);
    const transactions = new TransactionsService(database);
    const wishlists = new WishlistsService(database);
    ...
    assert(sentAll.every((offer: any) => typeof offer.game_id === "string"), "Sent offers must expose authoritative game_id.");
    ...
    assert(receivedAll.every((offer: any) => typeof offer.game_id === "string"), "Received offers must expose authoritative game_id.");
    ...
    if (wishlistSentAll.length > 0) {
      assert(wishlistSentAll.every((offer: any) => typeof offer.game_id === "string"), "Sent wishlist offers must expose authoritative game_id.");
    ...
    assert(transactionsAll.every((t: any) => typeof t.game_id === "string"), "Transactions must expose authoritative game_id.");
