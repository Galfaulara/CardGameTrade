...
import { MarketPricesService } from "../src/catalog/market-prices.service";
...
async function main() {
  ...
  const service = new MarketPricesService({ client: db } as never);
  const prefix = `pricing-regression-${Date.now()}`;
  const listingPricesBefore = JSON.stringify(
    await db.listings.findMany({
      orderBy: { id: "asc" },
  ...
  try {
    ...
    assert(
      rows.length === 3,
      "Daily retry must be idempotent while preserving prior history and separate providers.",
    );
    assert(
      JSON.stringify(
        await db.listings.findMany({
      ...
      "Market snapshots must not populate or mutate seller asking prices.",
    );
    ...
    assert(
      latest
        ...
        ?.market_price?.toString() === "8.71",
      "Latest exact-finish TCGplayer snapshot was not selected.",
    );
    assert(
      latest
        .find((row) => row.source === "cardkingdom")
        ?.market_price?.toString() === "9.99",
      "Card Kingdom source was not kept separate.",
    );
    ...
    assert(
      (await db.price_snapshots.count({
        where: { source_ref: { startsWith: prefix } },
      })) === countBeforeFailure,
      "Failed provider work erased valid snapshots.",
    );
    const targetTotal = 2 * 5;
    const offeredTotal = 13.2;
    const difference = offeredTotal - targetTotal;
    assert(
      targetTotal === 10 && Math.abs(difference - 3.2) < 0.0001,
      "Quantity totals or offered-minus-target difference changed.",
    );
    const incomplete = [5, null];
    assert(
      incomplete.some((value) => value === null),
      "Missing provider prices must suppress a complete total/difference instead of becoming zero.",
    );
