...
function unmappedPrincipal(seed: string): AuthenticatedPrincipal {
  return {
    provider: "clerk",
...
export async function runFridayTradeMvpRegression() {
  ...
  try {
    ...
    const sellerInventory = await createInventory(
      harness,
      user1,
      `${seed}-seller`,
    );
    const offererInventory = await createInventory(
      harness,
      user2,
      `${seed}-offerer`,
    );
    const storeOwnedCandidate = await createInventory(
      harness,
      user2,
      `${seed}-store`,
    );
    ...
    await harness.as(null).post(`/api/listings/users/${USER_1_ID}`).send({
    ...
    await harness.as(unmappedPrincipal(`${seed}-unmapped-listing`)).post(`/api/listings/users/${USER_1_ID}`).send({
      inventoryItemId: sellerInventory.inventoryId,
      acceptsCash: false,
      acceptsTrade: true,
    }).expect(403);
    ...
    await harness.as(disabled).post(`/api/listings/users/${USER_1_ID}`).send({
      inventoryItemId: sellerInventory.inventoryId,
      acceptsCash: false,
      acceptsTrade: true,
    }).expect(403);
    ...
    assert(listingResponse.body.seller_user_id === USER_1_ID, "Created listing must belong to User1.");
    ...
    const ownerListings = await harness.as(user1).get(`/api/listings/users/${USER_1_ID}`).expect(200);
    assert(ownerListings.body.some((listing: any) => listing.id === listingId), "User1 listings must include the new active trade listing.");
    ...
    const publicCardListings = await harness.as(null).get(
      `/api/catalog/cards/${sellerInventory.canonicalCardId}/listings?printing=${sellerInventory.printingId}&page=1&pageSize=12`,
    ).expect(200);
    const exactCardListing = publicCardListings.body.items.find((listing: any) => listing.id === listingId);
    ...
    await harness.as(user2).patch(`/api/listings/users/${USER_2_ID}/${listingId}/status`).send({
      status: "removed",
    }).expect(404);
    ...
    assert(
      JSON.stringify(afterOtherUserCancel) === JSON.stringify(beforeOtherUserCancel),
      "Cross-user listing cancellation must not mutate the listing.",
    );

    const beforeAnonymousOffer = await tableCounts();
    await harness.as(null).post(`/api/offers/listings/${listingId}/users/${USER_2_ID}`).send({
      cashAmount: 0,
      currencyCode: "USD",
    ...
    await harness.as(user1).post(`/api/offers/listings/${listingId}/users/${USER_1_ID}`).send({
      cashAmount: 0,
      currencyCode: "USD",
    ...
    await harness.as(user2).post(`/api/offers/listings/${listingId}/users/${USER_2_ID}`).send({
      cashAmount: 0,
      currencyCode: "USD",
      items: [{ inventoryItemId: sellerInventory.inventoryId, quantity: 1 }],
    }).expect(400);
    ...
    const offerResponse = await harness.as(user2).post(`/api/offers/listings/${listingId}/users/${USER_2_ID}`).send({
      cashAmount: 0,
      currencyCode: "USD",
      items: [{ inventoryItemId: offererInventory.inventoryId, quantity: 1 }],
    }).expect(201);
    ...
    assert(offerResponse.body.offerer_user_id === USER_2_ID, "Offer must be owned by User2.");
    ...
    assert(sellerInventoryRow?.owner_user_id === USER_1_ID, "Target inventory must remain owned by User1 after a pending offer.");
    ...
    assert(offererInventoryRow?.owner_user_id === USER_2_ID, "Offered inventory must remain owned by User2 after a pending offer.");
    ...
    assert(listingRow?.seller_user_id === USER_1_ID && listingRow.seller_store_id === null, "Pending offer must not change listing ownership.");
    assert(listingRow.status === "active", "Pending offer must not close the active listing.");

    const sentOffers = await harness.as(user2).get(`/api/offers/users/${USER_2_ID}/sent`).expect(200);
    assert(sentOffers.body.some((offer: any) => offer.id === offerId), "User2 sent offers must include the persisted pending offer.");

    const cancelledListing = await harness.as(user1).patch(`/api/listings/users/${USER_1_ID}/${listingId}/status`).send({
      status: "removed",
    }).expect(200);
    assert(cancelledListing.body.status === "removed", "User1 must be able to cancel the active listing using status semantics.");

    const ownerListingsAfterCancel = await harness.as(user1).get(`/api/listings/users/${USER_1_ID}`).expect(200);
    assert(!ownerListingsAfterCancel.body.some((listing: any) => listing.id === listingId), "Cancelled listings must leave the active My Listings read model.");
