...
const created = {
  ...
  temporaryStaffMemberships: [] as string[],
  temporaryStores: [] as string[],
};
...
const sameCounts = async (
  ...
  assert(
    ...
    `${message}: ${JSON.stringify({ before, after })}`,
  );
};
...
async function createInventory(
  harness: Awaited<ReturnType<typeof createAuthenticatedHarness>>,
  ...
  return {
    inventoryId: response.body.id as string,
    printingId: physical.printing_id,
...
async function createListing(
  ...
  const response = await harness.as(activePrincipal(USER_1_ID)).post(`/api/listings/users/${USER_1_ID}`).send({
    inventoryItemId,
    acceptsCash: false,
    acceptsTrade: true,
    preferredStoreId,
...
async function createOffer(
  ...
  const response = await harness.as(activePrincipal(USER_2_ID)).post(`/api/offers/listings/${listingId}/users/${USER_2_ID}`).send({
    cashAmount: 0,
    currencyCode: "USD",
...
async function inventorySnapshot(inventoryId: string) {
...
export async function runFridayTradeMvpRegression() {
  await db.$connect();
  ...
  const seed = `friday-trade-${randomUUID()}`;
  ...
  try {
    ...
    if (!wrongStaff) {
      const membership = await db.store_staff.create({
        data: { store_id: otherStore.id, user_id: USER_2_ID, role: "staff", status: "active" },
        select: { id: true, user_id: true },
      });
      created.temporaryStaffMemberships.push(membership.id);
      wrongStaff = membership;
    }
    const wrongStaffPrincipal = activePrincipal(wrongStaff.user_id);
    ...
    const rejectListingId = await createListing(
      harness,
      rejectSellerInventory.inventoryId,
      staff.store_id,
      `${seed}-reject`,
    );
    const acceptListingId = await createListing(
      harness,
      acceptSellerInventory.inventoryId,
      staff.store_id,
      `${seed}-accept`,
    );
    const staleListingId = await createListing(
      harness,
      staleSellerInventory.inventoryId,
      staff.store_id,
      `${seed}-stale`,
    );

    const rejectOfferId = await createOffer(
      harness,
      rejectListingId,
      rejectOffererInventory.inventoryId,
      `${seed}-reject`,
    );
    const acceptOfferId = await createOffer(
      harness,
      acceptListingId,
      acceptOffererInventory.inventoryId,
      `${seed}-accept`,
    );
    const staleOfferId = await createOffer(
      harness,
      staleListingId,
      staleOffererInventory.inventoryId,
      `${seed}-stale`,
    );

    for (const [actor, ownerId, inventoryId, suffix] of [
      [user1, USER_1_ID, acceptSellerInventory.inventoryId, "seller"],
    ...
    const sellerReceived = await harness.as(user1).get(`/api/offers/users/${USER_1_ID}/received`).expect(200);
    assert(sellerReceived.body.some((offer: any) => offer.id === rejectOfferId), "Seller received offers must include the reject candidate.");
    assert(sellerReceived.body.some((offer: any) => offer.id === acceptOfferId), "Seller received offers must include the accept candidate.");
    assert(sellerReceived.body.some((offer: any) => offer.id === staleOfferId), "Seller received offers must include the stale candidate.");
    const sellerPerListing = await harness.as(user1).get(`/api/offers/listings/${rejectListingId}/users/${USER_1_ID}/received`).expect(200);
    ...
    await harness.as(staffPrincipal).get(`/api/offers/users/${USER_1_ID}/received`).expect(403);
    ...
    await harness.as(user2).patch(`/api/offers/${rejectOfferId}/users/${USER_2_ID}/reject`).expect(404);
    ...
    const rejected = await harness.as(user1).patch(`/api/offers/${rejectOfferId}/users/${USER_1_ID}/reject`).expect(200);
    ...
    const sentAfterReject = await harness.as(user2).get(`/api/offers/users/${USER_2_ID}/sent`).expect(200);
    assert(sentAfterReject.body.find((offer: any) => offer.id === rejectOfferId)?.status === "rejected", "Sent offers must reflect rejected status after refresh.");

    const beforeUnauthorizedAccept = await tableCounts();
    await harness.as(user2).post(`/api/offers/${acceptOfferId}/users/${USER_2_ID}/accept`).send({ storeId: staff.store_id }).expect(403);
    ...
    await harness.as(user1).post(`/api/offers/${acceptOfferId}/users/${USER_1_ID}/accept`).send({ storeId: staff.store_id, sellerUserId: USER_2_ID }).expect(400);
    ...
    assert(accepted.body.seller_user_id === USER_1_ID, "Accepted transaction must record the seller participant.");
    assert(accepted.body.counterparty_user_id === USER_2_ID, "Accepted transaction must record the counterparty participant.");
    ...
    assert(acceptedListingRow.seller_user_id === USER_1_ID && acceptedListingRow.seller_store_id === null, "Accept must not change listing ownership.");
    ...
    assert(acceptedTransaction.seller_user_id === USER_1_ID && acceptedTransaction.counterparty_user_id === USER_2_ID, "Transaction participants are incorrect.");
    ...
    assert(listedItem.from_user_id === USER_1_ID && listedItem.to_user_id === USER_2_ID, "Listed transaction item participants are incorrect.");
    ...
    await harness.as(user1).post(`/api/offers/${acceptOfferId}/users/${USER_1_ID}/accept`).send({ storeId: staff.store_id }).expect(409);
    ...
    const sellerTransactionList = await harness.as(user1).get(`/api/transactions/users/${USER_1_ID}`).expect(200);
    assert(sellerTransactionList.body.some((trade: any) => trade.id === transactionId), "Seller My Trades read must include the accepted trade.");
    const offererTransactionList = await harness.as(user2).get(`/api/transactions/users/${USER_2_ID}`).expect(200);
    assert(offererTransactionList.body.some((trade: any) => trade.id === transactionId), "Offerer My Trades read must include the accepted trade.");

    const sellerTradeDetail = await harness.as(user1).get(`/api/transactions/${transactionId}/users/${USER_1_ID}`).expect(200);
    const offererTradeDetail = await harness.as(user2).get(`/api/transactions/${transactionId}/users/${USER_2_ID}`).expect(200);
    ...
    assert(sellerTradeDetail.body.items.filter((item: any) => item.from_user_id === USER_1_ID).length === 1, "Seller trade detail must expose exactly one give-side item.");
    assert(sellerTradeDetail.body.items.filter((item: any) => item.to_user_id === USER_1_ID).length === 1, "Seller trade detail must expose exactly one receive-side item.");
    assert(offererTradeDetail.body.items.filter((item: any) => item.from_user_id === USER_2_ID).length === 1, "Offerer trade detail must expose exactly one give-side item.");
    assert(offererTradeDetail.body.items.filter((item: any) => item.to_user_id === USER_2_ID).length === 1, "Offerer trade detail must expose exactly one receive-side item.");
    assert(sellerTradeDetail.body.items.some((item: any) => item.from_user_id === USER_1_ID && item.inventory_item_id === acceptSellerInventory.inventoryId), "Seller perspective must give the listed inventory item.");
    assert(sellerTradeDetail.body.items.some((item: any) => item.to_user_id === USER_1_ID && item.inventory_item_id === acceptOffererInventory.inventoryId), "Seller perspective must receive the offered inventory item.");
    assert(offererTradeDetail.body.items.some((item: any) => item.from_user_id === USER_2_ID && item.inventory_item_id === acceptOffererInventory.inventoryId), "Offerer perspective must give the offered inventory item.");
    assert(offererTradeDetail.body.items.some((item: any) => item.to_user_id === USER_2_ID && item.inventory_item_id === acceptSellerInventory.inventoryId), "Offerer perspective must receive the listed inventory item.");

    const sentAfterAccept = await harness.as(user2).get(`/api/offers/users/${USER_2_ID}/sent`).expect(200);
    assert(sentAfterAccept.body.find((offer: any) => offer.id === acceptOfferId)?.status === "accepted", "Sent offers must reflect accepted status after refresh.");
    const receivedAfterActions = await harness.as(user1).get(`/api/offers/users/${USER_1_ID}/received`).expect(200);
    assert(receivedAfterActions.body.find((offer: any) => offer.id === rejectOfferId)?.status === "rejected", "Received offers must reflect rejected status after refresh.");
    assert(receivedAfterActions.body.find((offer: any) => offer.id === acceptOfferId)?.status === "accepted", "Received offers must reflect accepted status after refresh.");
    ...
    await harness.as(user1).post(`/api/offers/${staleOfferId}/users/${USER_1_ID}/accept`).send({ storeId: staff.store_id }).expect(409);
    ...
    assert(staffList.body.some((row: any) => row.id === handoff.id), "Authorized Store staff must see the exact accepted handoff.");
    assert(!staffList.body.some((row: any) => row.store_id !== staff.store_id), "Store handoff list leaked another Store's work.");
    ...
    const resultIds = releasedItems.map((item) => item.result_inventory_item_id).filter((id): id is string => Boolean(id));
    ...
    for (const result of results) {
      ...
      for (const field of ["printing_id", "finish", "condition", "language_code", "quantity", "is_signed", "is_altered", "is_graded", "grading_company", "grade", "certification_number"] as const) {
    ...
    assert(results.filter((row) => row.owner_user_id === USER_1_ID).every((row) => sellerInventoryRead.body.items.some((item: any) => item.id === row.id)), "Seller recipient does not see newly received current Inventory.");
    assert(results.filter((row) => row.owner_user_id === USER_2_ID).every((row) => offererInventoryRead.body.items.some((item: any) => item.id === row.id)), "Offerer recipient does not see newly received current Inventory.");
    assert(!sellerInventoryRead.body.items.some((item: any) => item.id === acceptSellerInventory.inventoryId), "Former seller source remains in current Inventory.");
    assert(!offererInventoryRead.body.items.some((item: any) => item.id === acceptOffererInventory.inventoryId), "Former offerer source remains in current Inventory.");
    ...
    assert(
      ...
      `Friday trade cleanup did not restore baseline counts: ${JSON.stringify({ baseline, restored })}`,
    );
    await harness.close();
    await db.$disconnect();
  }
