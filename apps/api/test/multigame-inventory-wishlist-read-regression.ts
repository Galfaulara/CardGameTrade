...
async function main() {
  ...
  try {
    ...
    const inventory = new InventoryService(database, {} as StorageService);
    const wishlists = new WishlistsService(database);
    const discovery = new DiscoveryService(database);
    ...
    for (const slug of ["pokemon", "yugioh"] as const) {
      ...
      assert(
        persistedTargets.every((item) =>
          item.game_id === gameId &&
          (!item.canonical_cards || item.canonical_cards.game_id === gameId) &&
          (!item.card_printings || item.card_printings.game_id === gameId)),
        `${slug} wishlist target game disagrees with its parent.`,
      );
    }
