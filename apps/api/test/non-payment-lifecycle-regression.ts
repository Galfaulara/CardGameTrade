...
type Created = { collections: string[]; inventories: string[]; listings: string[]; interests: string[]; offers: string[]; transactions: string[] };
const created: Created = { collections: [], inventories: [], listings: [], interests: [], offers: [], transactions: [] };
const BASELINE_TABLES = ["collections", "inventory_items", "listings", "listing_offers", "offer_items", "inventory_item_interests", "transactions", "transaction_items", "store_trade_handoffs", "transaction_item_custody"] as const;

async function tableCounts() {
  const counts: Record<string, number> = {};
  for (const table of BASELINE_TABLES) counts[table] = await (db as any)[table].count();
  return counts;
}
...
async function cleanup() {
  ...
  if (created.offers.length) await db.listing_offers.deleteMany({ where: { id: { in: created.offers } } });
...
export async function runNonPaymentLifecycleRegression() {
  await db.$connect();
  ...
  try {
    ...
    const marker = `Step 3B ${randomUUID()}`;
    ...
    const createInventory = async (actor: typeof user1, ownerId: string, collectionId: string, suffix: string) => {
    ...
    const sellerInventory = await createInventory(user1, USER_1_ID, sellerCollectionId, "seller");
    const offererInventory = await createInventory(user2, USER_2_ID, offererCollectionId, "offerer");

    const listingResponse = await harness.as(user1).post(`/api/listings/users/${USER_1_ID}`).send({
      inventoryItemId: sellerInventory, acceptsCash: false, acceptsTrade: true,
      preferredStoreId: staff.store_id, title: marker,
    }).expect(201);
    const listingId = listingResponse.body.id as string;
    created.listings.push(listingId);

    const interestResponse = await harness.as(user2)
      .post(`/api/inventory-interests/inventory/${sellerInventory}/users/${USER_2_ID}`)
      .send({ interestType: "trade", message: marker }).expect(201);
    const interestId = interestResponse.body.id as string;
    created.interests.push(interestId);

    const offerResponse = await harness.as(user2)
      .post(`/api/offers/listings/${listingId}/users/${USER_2_ID}/from-interest/${interestId}`)
    ...
    await harness.as(user2).post(`/api/offers/${offerId}/users/${USER_2_ID}/accept`).send({ storeId: staff.store_id }).expect(403);
    const accepted = await harness.as(user1).post(`/api/offers/${offerId}/users/${USER_1_ID}/accept`)
    ...
    assert(transaction?.seller_user_id === USER_1_ID && transaction.counterparty_user_id === USER_2_ID, "Transaction actors are incorrect.");
    ...
    await harness.as(user1).patch(`/api/store-handoffs/${handoff.id}/items/${firstTransactionItem.id}/receive/users/${USER_1_ID}`).expect(403);
    ...
    for (const item of releasedItems) assert(item.result_inventory_item_id, "Released transaction item lacks result inventory.");
    ...
    const user2Received = releasedItems.find((item) => item.to_user_id === USER_2_ID)?.result_inventory_item_id;
    const user1Received = releasedItems.find((item) => item.to_user_id === USER_1_ID)?.result_inventory_item_id;
    assert(user2Received && user1Received, "First-generation recipient inventory could not be resolved.");

    await harness.as(user2).patch(`/api/inventory/users/${USER_2_ID}/items/${user2Received}/collection`)
      .send({ collectionId: created.collections[1] }).expect(200);
    await harness.as(user1).patch(`/api/inventory/users/${USER_1_ID}/items/${user1Received}/collection`)
      .send({ collectionId: created.collections[0] }).expect(200);

    const secondListingResponse = await harness.as(user2).post(`/api/listings/users/${USER_2_ID}`).send({
    ...
    const secondListingId = secondListingResponse.body.id as string;
    created.listings.push(secondListingId);
    const secondInterestResponse = await harness.as(user1)
      .post(`/api/inventory-interests/inventory/${user2Received}/users/${USER_1_ID}`)
      .send({ interestType: "trade", message: `${marker} generation 2` }).expect(201);
    const secondInterestId = secondInterestResponse.body.id as string;
    created.interests.push(secondInterestId);
    const secondOfferResponse = await harness.as(user1)
      .post(`/api/offers/listings/${secondListingId}/users/${USER_1_ID}/from-interest/${secondInterestId}`)
      .send({ cashAmount: 0, currencyCode: "USD", items: [{ inventoryItemId: user1Received, quantity: 1 }] }).expect(201);
    const secondOfferId = secondOfferResponse.body.id as string;
    created.offers.push(secondOfferId);
    const secondAccepted = await harness.as(user2).post(`/api/offers/${secondOfferId}/users/${USER_2_ID}/accept`)
      .send({ storeId: staff.store_id }).expect(201);
    const secondTransactionId = secondAccepted.body.id as string;
    ...
    for (const item of secondItems) await harness.as(staffPrincipal).patch(`/api/store-handoffs/${secondHandoff.id}/items/${item.id}/receive/users/${staff.user_id}`).expect(200);
    for (const item of secondItems) await harness.as(staffPrincipal).patch(`/api/store-handoffs/${secondHandoff.id}/items/${item.id}/verify/users/${staff.user_id}`).expect(200);
    for (const item of secondItems) await harness.as(staffPrincipal).patch(`/api/store-handoffs/${secondHandoff.id}/items/${item.id}/release/users/${staff.user_id}`).expect(200);
    const secondReleased = await db.transaction_items.findMany({ where: { transaction_id: secondTransactionId } });
    const secondResultIds = secondReleased.map((item) => item.result_inventory_item_id).filter((id): id is string => Boolean(id));
