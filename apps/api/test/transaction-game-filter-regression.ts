...
function matchesWhere(row: Record<string, unknown>, where: Record<string, unknown>): boolean {
  return Object.keys(where).every((key) => {
    if (key === "OR") {
      const options = where.OR as Array<Record<string, unknown>>;
    ...
    client: {
      ...
      transactions: {
        findMany: async ({ where }: { where: Record<string, unknown> }) =>
          transactions.filter((transaction) => matchesWhere(transaction, where)),
        findFirst: async ({ where }: { where: Record<string, unknown> }) =>
          transactions.find((transaction) => matchesWhere(transaction, where)) ?? null,
      },
...
async function main() {
  const service = new TransactionsService(fakeDatabase());
  ...
  assert(
    all.map((t: any) => t.id).sort().join(",") === ["txn-mtg", "txn-pokemon"].sort().join(","),
    "Unfiltered transaction history must preserve current behavior (both seller and counterparty roles, all games).",
  );
  ...
  for (const list of [all, mtgOnly, pokemonOnly, yugiohOnly]) {
    assert(!list.some((t: any) => t.id === "txn-other-user"), "The game filter must never surface a transaction the user isn't a participant in.");
  }
  ...
  try {
    ...
    detailThrew = error instanceof NotFoundException;
  }
