...
async function main() {
  ...
  try {
    ...
    const collectionPage = await html(`/collections/${collection.id}`); assert(collectionPage.includes(`/users/${collection.owner.id}`) && collectionPage.includes("/cards/") && collectionPage.includes("printing="), "Collection graph missing"); if (collection.preferred_store) assert(collectionPage.includes(`/stores/${collection.preferred_store.id}`), "Collection preferred Store link missing");
    const userPage = await html(`/users/${collection.owner.id}`); assert(userPage.includes(`/collections/${collection.id}`) && userPage.includes("/cards/"), "User graph missing"); if (collection.preferred_store) assert(userPage.includes(`/stores/${collection.preferred_store.id}`), "User preferred Store link missing");
    const storePage = await html(`/stores/${store.id}`); assert(storePage.includes("/cards/"), "Store graph missing");
    const cardPage = await html(printingHref); assert(cardPage.includes("scryfall.com") && (cardPage.includes("/users/") || cardPage.includes("/stores/")), "Card seller/Scryfall graph missing");
