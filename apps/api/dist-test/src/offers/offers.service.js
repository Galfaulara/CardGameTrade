...
let OffersService = class OffersService {
    ...
    mapOffer(offer) {
        const { offer_items_offer_items_offer_idTolisting_offers: rawItems, ...offerData } = offer;
        const items = rawItems.map((item) => {
            const inventoryItem = item
                .inventory_items_offer_items_inventory_item_id_offerer_user_idToinventory_items ??
                item
                    .inventory_items_offer_items_inventory_item_id_offerer_store_idToinventory_items;
            const { inventory_items_offer_items_inventory_item_id_offerer_user_idToinventory_items: _userRelation, inventory_items_offer_items_inventory_item_id_offerer_store_idToinventory_items: _storeRelation, ...itemData } = item;
            if (!inventoryItem) {
                return {
                    ...itemData,
                    inventory_item: null,
    ...
    async loadOffer(offerId) {
        const offer = await this.database.client.listing_offers.findUnique({
            where: {
                id: offerId,
            },
        ...
        if (!offer) {
            throw new common_1.NotFoundException("Offer was not found.");
        }
        return this.mapOffer(offer);
    }
    async getAcceptedTransaction(transactionId) {
        const transaction = await this.database.client.transactions.findUnique({
            where: {
                id: transactionId,
            },
            select: {
    ...
    async createUserOffer(userId, listingId, input, interestId) {
        ...
        if (user.status !== "active") {
            throw new common_1.ForbiddenException("Offers cannot be created for an inactive user.");
        }
        ...
        if (listing.status !== "active") {
            throw new common_1.BadRequestException("Offers can only be made on active listings.");
        }
        if (listing.seller_user_id ===
            userId) {
            throw new common_1.BadRequestException("A user cannot make an offer on their own listing.");
        }
        const hasCash = input.cashAmount > 0;
        const hasTradeItems = input.items.length > 0;
        if (hasCash &&
        ...
        if (hasTradeItems &&
            ...
            throw new common_1.BadRequestException("This listing does not accept card trades.");
        }
        if (input.expiresAt &&
            input.expiresAt <=
                new Date()) {
            throw new common_1.BadRequestException("Offer expiration must be in the future.");
        }
        if (hasTradeItems) {
            const inventoryIds = input.items.map((item) => item.inventoryItemId);
            const inventoryItems = await this.database.client.inventory_items.findMany({
                where: {
                    id: {
                        in: inventoryIds,
                    },
            ...
            if (inventoryItems.length !==
                inventoryIds.length) {
                throw new common_1.BadRequestException("Every offered card must exist in the offerer's inventory.");
            }
            const inventoryById = new Map(inventoryItems.map((item) => [
                item.id,
                item,
            ]));
            for (const requestedItem of input.items) {
                const inventoryItem = inventoryById.get(requestedItem.inventoryItemId);
                if (!inventoryItem) {
                ...
                if (requestedItem.quantity !==
                    ...
                    throw new common_1.BadRequestException("An offered inventory item must include its full registered quantity. Split the inventory item first if only part of the quantity should be offered.");
                }
            }
        }
        const offerId = await this.database.client.$transaction(async (transaction) => {
            ...
            if (interestId) {
                ...
                if (interest.interested_user_id !==
                    ...
                    interest.interested_store_id !==
                        null) {
                    throw new common_1.ForbiddenException("Only the user who created this interest can convert it into a listing offer.");
                }
                if (interest.inventory_item_id !==
                    listing.inventory_item_id) {
                    throw new common_1.BadRequestException("This interest does not target the inventory item used by this listing.");
                }
                if (interest.status !==
                    "active") {
                ...
                if (interest.interest_type ===
                    "buy" &&
                    hasTradeItems) {
                    throw new common_1.BadRequestException("A buy-only interest cannot be converted into an offer containing trade items.");
                }
                if (interest.interest_type ===
                    "trade" &&
                    hasCash) {
                    throw new common_1.BadRequestException("A trade-only interest cannot be converted into an offer containing cash.");
                }
            }
            ...
            if (interestId) {
                ...
                if (converted.count !==
                    1) {
                    throw new common_1.ConflictException("The inventory-item interest changed while the listing offer was being created.");
                }
            }
            return offer.id;
        });
    ...
    async withdrawUserOffer(userId, offerId) {
        const offer = await this.database.client.listing_offers.findFirst({
            where: {
                id: offerId,
                offerer_user_id: userId,
            },
        ...
        if (!offer) {
            throw new common_1.NotFoundException("Offer was not found or does not belong to this user.");
        }
        if (offer.status !==
            "pending") {
    ...
    async rejectUserOffer(sellerUserId, offerId) {
        const offer = await this.database.client.listing_offers.findUnique({
            where: {
                id: offerId,
            },
    ...
    async acceptUserOffer(sellerUserId, offerId, input) {
        const transactionId = await this.database.client.$transaction(async (transaction) => {
            const now = new Date();
            const offer = await transaction.listing_offers.findUnique({
                where: {
                    id: offerId,
                },
                select: {
            ...
            if (offer.status !==
                "pending") {
                throw new common_1.ConflictException("This offer is no longer pending.");
            }
            if (offer.expires_at &&
                offer.expires_at <=
                    now) {
            ...
            if (!offer.offerer_user_id ||
                ...
                throw new common_1.BadRequestException("Store-created offer acceptance is not implemented yet.");
            }
            const listing = await transaction.listings.findUnique({
                where: {
                    id: offer.listing_id,
                },
                select: {
            ...
            if (listing.seller_user_id !==
                ...
                listing.seller_store_id !==
                    null) {
                throw new common_1.ForbiddenException("Only the user who owns this listing can accept the offer.");
            }
            if (listing.status !==
                "active") {
                throw new common_1.ConflictException("The listing is no longer active.");
            }
            ...
            if (!store) {
                throw new common_1.BadRequestException("The selected store is not currently eligible to mediate DeckDeal trades.");
            }
            const listedInventory = await transaction.inventory_items.findFirst({
                where: {
            ...
            if (!listedInventory) {
                throw new common_1.ConflictException("The listed inventory item no longer belongs to the seller.");
            }
            ...
            const offeredInventoryIds = offerItems.map((item) => item.inventory_item_id);
            const offeredInventory = offeredInventoryIds.length >
                0
            ...
            const offeredInventoryById = new Map(offeredInventory.map((item) => [
                item.id,
                item,
            ]));
            for (const offerItem of offerItems) {
                const inventoryItem = offeredInventoryById.get(offerItem.inventory_item_id);
                if (!inventoryItem) {
                ...
                if (offerItem.quantity !==
                    inventoryItem.quantity) {
                    throw new common_1.ConflictException("An offered inventory quantity no longer matches the registered inventory lot.");
                }
            }
            const claimedListing = await transaction.listings.updateMany({
                where: {
                    id: listing.id,
                    status: "active",
                },
            ...
            if (claimedListing.count !==
                1) {
                throw new common_1.ConflictException("The listing changed while the offer was being accepted.");
            }
            const claimedOffer = await transaction.listing_offers.updateMany({
                where: {
                    id: offer.id,
                    status: "pending",
                },
            ...
            if (claimedOffer.count !==
                1) {
                throw new common_1.ConflictException("The offer changed while it was being accepted.");
            }
            ...
            const hasCash = Number(offer.cash_amount) > 0;
            const hasTradeItems = offerItems.length > 0;
            /*
            ...
            const listedTransactionItem = await transaction.transaction_items.create({
                data: {
                    ...
                    quantity: listedInventory.quantity,
            ...
            for (const offerItem of offerItems) {
                ...
                offeredTransactionItemIds.push(transactionItem.id);
            }
            const listedReservation = await transaction.inventory_items.updateMany({
                where: {
            ...
            if (listedReservation.count !==
                1) {
                throw new common_1.ConflictException("The listed card could not be reserved for the trade.");
            }
            for (const inventoryItem of offeredInventory) {
                const reservation = await transaction.inventory_items.updateMany({
                    where: {
                        id: inventoryItem.id,
                        owner_user_id: offer.offerer_user_id,
                        status: "available",
                ...
                if (reservation.count !==
                    1) {
                    throw new common_1.ConflictException("An offered card could not be reserved for the trade.");
                }
            }
        ...
        return this.getAcceptedTransaction(transactionId);
    }
};
