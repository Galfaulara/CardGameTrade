...
const eligibleStore = { status: "active", verification_status: "verified", trade_mediation_enabled: true };
...
let DiscoveryService = class DiscoveryService {
    ...
    decodeCursor(cursor, kind, filter, gameId) {
        if (!cursor)
            return null;
        try {
            ...
            throw new common_1.BadRequestException("The discovery cursor is malformed or does not match these filters.");
        }
    }
    mapItem(item) {
    ...
    mapPreferredStore(store) {
        if (!store || store.status !== "active" || store.verification_status !== "verified" || !store.trade_mediation_enabled)
            return null;
    ...
    userListingWhere(userId, gameId) {
    ...
    storeListingWhere(storeId, gameId) {
    ...
    async requirePublicStore(id) {
        ...
        if (!store)
            throw new common_1.NotFoundException("Store was not found.");
        return store;
    }
    ...
    async getUserCollections(id, query) {
        const user = await this.requirePublicUser(id);
        const where = { user_id: id, visibility: "public" };
        ...
        const allById = new Map(allCounts.map((value) => [value.collection_id, value]));
        const marketById = new Map(marketCounts.map((value) => [value.collection_id, value]));
    ...
    async getUserWishlists(id, query) {
        const user = await this.requirePublicUser(id);
        const gameId = await this.resolveGameId(query.gameSlug);
        ...
        const countsById = new Map(counts.map((value) => [value.wishlist_id, value]));
        return { user: { id: user.id, display_name: user.display_name, username: user.username },
    ...
    async rankedPreviews(kind, ids, limit) {
        if (!ids.length)
            return new Map();
        const owner = kind === "collection" ? db_1.Prisma.raw("i.collection_id") : db_1.Prisma.raw("i.owner_store_id");
        const ownership = kind === "collection"
            ? db_1.Prisma.sql `l.seller_user_id = i.owner_user_id AND l.seller_store_id IS NULL`
            : db_1.Prisma.sql `l.seller_store_id = i.owner_store_id AND l.seller_user_id IS NULL`;
        const ranked = await this.database.client.$queryRaw(db_1.Prisma.sql `
      SELECT listing_id, owner_id, preview_rank FROM (
        ...
        WHERE ${owner} IN (${db_1.Prisma.join(ids)}) AND i.status = 'available'
    ...
    async getCollectionFeed(query) {
        const gameId = await this.resolveGameId(query.gameSlug);
        const cursor = this.decodeCursor(query.cursor, "collections", query.availability, gameId);
    ...
    async getStoreFeed(query) {
        const cursor = this.decodeCursor(query.cursor, "stores", query.availability);
        ...
        const allById = new Map(allCounts.map((value) => [value.owner_store_id, value]));
        const marketById = new Map(marketCounts.map((value) => [value.owner_store_id, value]));
    ...
    async getListingFeed(query) {
        const gameId = await this.resolveGameId(query.gameSlug);
        const cursor = this.decodeCursor(query.cursor, "listings", query.intent, gameId);
        ...
        const where = { status: "active", ...(gameId ? { game_id: gameId } : {}), AND: [intent, ownership,
    ...
    async getCollections(query) {
        ...
        const rankedCandidates = [...candidateById.values()].sort((a, b) => b.updated_at.getTime() - a.updated_at.getTime() || a.id.localeCompare(b.id));
        ...
        const collection = await this.database.client.collections.findFirst({ where: { id, visibility: "public", user_profiles: { status: "active" } },
            select: { id: true, game_id: true, name: true, description: true, created_at: true, updated_at: true,
                user_profiles: { select: { id: true, display_name: true, username: true,
                        preferred_store: { select: preferredStoreSelect } } } } });
        if (!collection)
            throw new common_1.NotFoundException("Collection was not found.");
        const where = { collection_id: id, owner_user_id: collection.user_profiles.id, ...available };
    ...
    async getStore(id) {
        ...
        const inventoryWhere = { owner_store_id: id, owner_user_id: null, ...available };
    ...
    async getStoreListings(id, query) {
        const store = await this.requirePublicStore(id);
        const gameId = await this.resolveGameId(query.gameSlug);
    ...
    async getStoreInventory(id, query) {
        const store = await this.requirePublicStore(id);
        const where = { owner_store_id: id, owner_user_id: null, ...available };
        const [items, aggregate] = await Promise.all([
