...
async function counts() {
    ...
    for (const table of tables)
        result[table] = await db[table].count();
    return result;
}
async function assertCounts(before, names, message) {
    const after = await counts();
    for (const name of names)
        assert(after[name] === before[name], `${message}: ${name} changed (${before[name]} -> ${after[name]}).`);
}
async function cleanup() {
    if (created.transactions.length)
        await db.transactions.deleteMany({ where: { id: { in: created.transactions } } });
    if (created.wishlistOffers.length)
        await db.wishlist_offers.deleteMany({ where: { id: { in: created.wishlistOffers } } });
    if (created.listingOffers.length)
        await db.listing_offers.deleteMany({ where: { id: { in: created.listingOffers } } });
    ...
    if (created.wishlists.length)
        await db.wishlists.deleteMany({ where: { id: { in: created.wishlists } } });
    ...
    if (created.storeId)
        await db.stores.deleteMany({ where: { id: created.storeId, slug: `${prefix}-unsupported-store` } });
    if (created.printingId)
        await db.card_printings.deleteMany({ where: { id: created.printingId, source_key: `${prefix}-printing` } });
    if (created.canonicalId)
        await db.canonical_cards.deleteMany({ where: { id: created.canonicalId, source_key: `${prefix}-card` } });
    if (created.setId)
        await db.card_sets.deleteMany({ where: { id: created.setId, source_id: `${prefix}-set` } });
    if (created.gameId)
        await db.games.deleteMany({ where: { id: created.gameId, slug: prefix } });
}
async function main() {
    await db.$connect();
    ...
    try {
        ...
        const mtgCollection = await harness.as(user1).post(`/api/inventory/users/${authenticated_app_harness_1.USER_1_ID}/collections`)
            .send({ gameSlug: "mtg", name: sharedName, visibility: "public" }).expect(201);
        created.collections.push(mtgCollection.body.id);
        const syntheticCollection = await harness.as(user1).post(`/api/inventory/users/${authenticated_app_harness_1.USER_1_ID}/collections`)
        ...
        await harness.as(user1).post(`/api/inventory/users/${authenticated_app_harness_1.USER_1_ID}/collections`)
            .send({ gameSlug: `${prefix}-missing`, name: `${prefix} Missing`, visibility: "private" }).expect(400);
        const inventoryBody = (printingId, finish, collectionId, notes) => ({
            printingId, finish, collectionId, condition: "near_mint", quantity: 1,
            isSigned: false, isAltered: false, isGraded: false, notes,
        });
        const createInventory = async (actor, printingId, finish, collectionId, note) => {
        ...
        await harness.as(user1).patch(`/api/inventory/users/${authenticated_app_harness_1.USER_1_ID}/items/${mtgSellerInventory}/collection`)
        ...
        const wishlist = await harness.as(user1).post(`/api/wishlists/users/${authenticated_app_harness_1.USER_1_ID}`)
        ...
        const syntheticWishlist = await harness.as(user1).post(`/api/wishlists/users/${authenticated_app_harness_1.USER_1_ID}`)
            .send({ gameSlug: prefix, name: `${prefix} Wishlist`, visibility: "private" }).expect(201);
        created.wishlists.push(syntheticWishlist.body.id);
        await harness.as(user1).post(`/api/wishlists/users/${authenticated_app_harness_1.USER_1_ID}`)
            .send({ gameSlug: "mtg", name: `${prefix} Unsupported Wishlist`, visibility: "private", preferredStoreId: unsupportedStore.id }).expect(400);
        const wishlistItem = await harness.as(user1).post(`/api/wishlists/${wishlist.body.id}/users/${authenticated_app_harness_1.USER_1_ID}/items`)
        ...
        await harness.as(user1).post(`/api/wishlists/${wishlist.body.id}/users/${authenticated_app_harness_1.USER_1_ID}/items`)
            .send({ printingId: printing.id, quantityDesired: 1, willingToPayCash: false, willingToTradeCards: true }).expect(400);
        await harness.as(user1).patch(`/api/wishlists/${wishlist.body.id}/items/${wishlistItem.body.id}/users/${authenticated_app_harness_1.USER_1_ID}`)
        ...
        const listing = await harness.as(user1).post(`/api/listings/users/${authenticated_app_harness_1.USER_1_ID}`).send({
        ...
        const interest = await harness.as(user2).post(`/api/inventory-interests/inventory/${mtgSellerInventory}/users/${authenticated_app_harness_1.USER_2_ID}`)
        ...
        await harness.as(user2).post(`/api/offers/listings/${listing.body.id}/users/${authenticated_app_harness_1.USER_2_ID}`).send({
        ...
        const listingOffer = await harness.as(user2).post(`/api/offers/listings/${listing.body.id}/users/${authenticated_app_harness_1.USER_2_ID}`).send({
        ...
        for (const body of negativeWishlistBodies) {
            ...
            await harness.as(user2).post(`/api/wishlists/items/${wishlistItem.body.id}/offers/users/${authenticated_app_harness_1.USER_2_ID}`).send({
        ...
        const wishlistOffer = await harness.as(user2).post(`/api/wishlists/items/${wishlistItem.body.id}/offers/users/${authenticated_app_harness_1.USER_2_ID}`).send({
        ...
        await harness.as(user1).post(`/api/wishlists/offers/${wishlistOffer.body.id}/users/${authenticated_app_harness_1.USER_1_ID}/accept`)
        ...
        const acceptedWishlistTransaction = await harness.as(user1).post(`/api/wishlists/offers/${wishlistOffer.body.id}/users/${authenticated_app_harness_1.USER_1_ID}/accept`)
