...
exports.runMyInventoryRegression = runMyInventoryRegression;
...
async function inventorySnapshot(inventoryItemId) {
...
async function runMyInventoryRegression() {
    await db.$connect();
    ...
    try {
        ...
        const searchTerm = physical.card_printings.canonical_cards.name
            .split(/\s+/)
            .filter(Boolean)[0] ??
        ...
        const publicCollection = await harness.as(user1).post(`/api/inventory/users/${authenticated_app_harness_1.USER_1_ID}/collections`).send({
            gameSlug: "mtg",
            name: `My Inventory ${seed.slice(0, 8)}`,
            visibility: "public",
        }).expect(201);
        cleanup.collections.push(publicCollection.body.id);
        const createBody = (note, condition, acquiredPrice, collectionId) => ({
        ...
        const beforeRejectedCreate = await db.inventory_items.count({
            where: {
                owner_user_id: authenticated_app_harness_1.USER_1_ID,
            },
        });
        await harness.as(user1).post("/api/me/inventory").send({
            ...
            owner_user_id: authenticated_app_harness_1.USER_2_ID,
        }).expect(400);
        await harness.as(user1).post("/api/me/inventory").send({
        ...
        const listedItem = await harness.as(user1).post("/api/me/inventory").send(createBody(listedNote, "near_mint", listedPrice, publicCollection.body.id)).expect(201);
        ...
        assert(createdListedRow?.owner_user_id === authenticated_app_harness_1.USER_1_ID, "New self inventory must be owned by the authenticated DeckDeal user.");
        ...
        const removableItem = await harness.as(user1).post("/api/me/inventory").send(createBody(removableNote, "damaged", "456.78", publicCollection.body.id)).expect(201);
        cleanup.inventories.push(removableItem.body.id);
        const user2Item = await harness.as(user2).post("/api/me/inventory").send(createBody(`inventory-user2-${seed}`, "near_mint", "111.22", null)).expect(201);
        ...
        assert(selfList.body.items.some((item) => item.id === listedItem.body.id), "Condition and search filters must be able to find the actor's own inventory item.");
        assert(!selfList.body.items.some((item) => item.id === user2Item.body.id), "User1's list must exclude User2's private inventory.");
        for (const catalogQuery of [
            ...
            assert(catalogFiltered.body.items.some((item) => item.id === listedItem.body.id), "Inventory search must truthfully support collector number and set identity.");
        }
        ...
        assert(user2List.body.items.some((item) => item.id === user2Item.body.id), "User2's list must contain User2's own matching inventory.");
        assert(!user2List.body.items.some((item) => item.id === listedItem.body.id), "User2's list must exclude User1's private inventory.");
        ...
        assert(collectionList.body.items.some((item) => item.id === listedItem.body.id), "Collection filtering must include matching actor-owned Inventory.");
        assert(!collectionList.body.items.some((item) => item.id === user2Item.body.id), "Collection filtering must remain actor-scoped.");
        const unassignedList = await harness.as(user2).get("/api/me/inventory?page=1&pageSize=48&collection=unassigned").expect(200);
        assert(unassignedList.body.items.some((item) => item.id === user2Item.body.id), "Unsorted must truthfully represent actor-owned rows with no collection.");
        const beforeRejectedListSpoof = await tableCounts();
        await harness.as(user1).get(`/api/me/inventory?page=1&pageSize=48&userId=${authenticated_app_harness_1.USER_2_ID}`).expect(400);
        ...
        await harness.as(user1).patch(`/api/me/inventory/${listedItem.body.id}`).send({ userId: authenticated_app_harness_1.USER_2_ID, notes: "spoofed" }).expect(400);
        ...
        assert(updatedFilter.body.items.some((item) => item.id === listedItem.body.id), "Updated condition filters must reflect self inventory edits.");
        ...
        const protectedItem = await harness.as(user1).post("/api/me/inventory").send(createBody(`inventory-protected-${seed}`, "near_mint", "222.33", null)).expect(201);
        ...
        assert(!postRemovalList.body.items.some((item) => item.id === removableItem.body.id), "Removed inventory must not remain in the current self inventory list.");
