...
const ids = (items) => items.map((item) => item.id).sort().join(",");
    ...
    return {
        client: {
            ...
            listings: {
                ...
                findMany: async ({ where, }) => listings.filter((listing) => listing.seller_user_id === where.seller_user_id &&
                    listing.seller_store_id === where.seller_store_id),
            },
            listing_offers: {
                findMany: async ({ where, }) => listingOffers.filter((offer) => {
                    if (where.offerer_user_id !== undefined && offer.offerer_user_id !== where.offerer_user_id)
                        return false;
                    if (where.game_id !== undefined && offer.game_id !== where.game_id)
                        return false;
                    if (where.listing_id && !where.listing_id.in.includes(offer.listing_id))
                        return false;
                    return true;
                }),
            },
...
async function testListingOffers() {
    const service = new offers_service_1.OffersService(fakeOffersDatabase());
    ...
    assert(sentMtg.length === 1 && sentMtg[0]?.id === "offer-sent-mtg" && sentMtg[0].game_id === "game-mtg", "mtg-filtered sent offers must return only the MTG offer and expose game_id.");
    ...
    assert(receivedMtg.length === 1 && receivedMtg[0]?.id === "offer-received-mtg" && receivedMtg[0].game_id === "game-mtg", "mtg-filtered received offers must return only the MTG offer and expose game_id.");
    ...
    try {
        ...
        receivedThrew = error instanceof common_1.BadRequestException;
    }
...
function fakeWishlistsDatabase() {
    return {
        client: {
            user_profiles: {
                findUnique: async ({ where: { id } }) => id === "user-1" ? { id: "user-1", status: "active" } : null,
                findMany: async () => [],
            },
            games: {
                findUnique: async ({ where: { slug } }) => games.find((game) => game.slug === slug) ?? null,
            ...
            wishlist_offers: {
                findMany: async ({ where, }) => wishlistOffers.filter((offer) => {
                    if (where.offerer_user_id !== undefined && offer.offerer_user_id !== where.offerer_user_id)
                        return false;
                    if (where.game_id !== undefined && offer.game_id !== where.game_id)
                        return false;
...
async function testWishlistOffers() {
    const service = new wishlists_service_1.WishlistsService(fakeWishlistsDatabase());
    ...
    assert(sentMtg.length === 1 && sentMtg[0]?.id === "wishlist-offer-mtg" && sentMtg[0].game_id === "game-mtg", "mtg-filtered sent wishlist offers must return only the MTG offer and expose game_id.");
