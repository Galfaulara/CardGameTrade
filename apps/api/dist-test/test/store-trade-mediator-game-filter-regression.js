...
function matchesWhere(store, where) {
    if (where.status !== undefined && store.status !== where.status)
        return false;
    if (where.verification_status !== undefined &&
        store.verification_status !== where.verification_status)
        return false;
    if (where.trade_mediation_enabled !== undefined &&
        store.trade_mediation_enabled !== where.trade_mediation_enabled)
        return false;
    const storeGames = where.store_games;
    if (storeGames) {
        const match = store.store_games.some((row) => row.game_id === storeGames.some.game_id &&
...
function fakeDatabase() {
    return {
        client: {
            games: {
                findUnique: async ({ where: { slug } }) => fixtureGames.find((game) => game.slug === slug) ?? null,
            },
            stores: {
                findMany: async ({ where, select, }) => fixtureStores
...
async function main() {
    const service = new stores_service_1.StoresService(fakeDatabase());
    ...
    assert(unfiltered.map((store) => store.id).sort().join(",") ===
        ["store-all-games", "store-mtg-only", "store-pokemon-mediation-disabled"].sort().join(","), "Unfiltered mediators must preserve the pre-existing global eligibility rule, independent of store_games.");
    ...
    assert(mtg.map((store) => store.id).sort().join(",") ===
        ["store-all-games", "store-mtg-only"].sort().join(","), "mtg filter must return only MTG-supported mediators.");
    ...
    assert(pokemon.length === 1 && pokemon[0]?.id === "store-all-games", "pokemon filter must return only Pokémon-supported mediators, excluding disabled/absent store_games.");
    ...
    assert(yugioh.length === 1 && yugioh[0]?.id === "store-all-games", "yugioh filter must return only Yu-Gi-Oh!-supported mediators.");
    // 5 (explicit). The unverified store must never appear, regardless of filter.
    for (const list of [unfiltered, mtg, pokemon, yugioh]) {
        assert(!list.some((store) => store.id === "store-unverified"), "An unverified store must be excluded from every mediator view (global and game-filtered).");
    }
    ...
    try {
        ...
        threwBadRequest = error instanceof common_1.BadRequestException;
    }
    ...
    for (const list of [unfiltered, mtg, pokemon, yugioh]) {
        for (const store of list) {
            assert(!Object.prototype.hasOwnProperty.call(store, "store_games"), "Trade-mediator responses must never leak the internal store_games relation.");
        }
    }
