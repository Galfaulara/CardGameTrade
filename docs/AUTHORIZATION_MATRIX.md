...
| GET | `/me/inventory` | AUTHENTICATED_USER_READ | Actor reads only current actor-owned, non-Store Inventory with bounded search/status/condition/Collection pagination and owner-private metadata; `collection=unassigned` means `collection_id IS NULL` | Self-scoped by guarded principal; service enforces `owner_user_id = actor`, `owner_store_id IS NULL`, and current lifecycle statuses; arbitrary Collection IDs cannot broaden the owner predicate |
| POST | `/me/inventory` | AUTHENTICATED_USER_WRITE | Actor registers an exact physical printing/finish as actor-owned Inventory | Strict DTO rejects owner fields; service assigns `owner_user_id = actor`, `owner_store_id = null`, validates exact physical printing/finish and actor-owned collection, and starts the row `available` without creating a Listing or marketplace intent |
| GET | `/me/inventory/:inventoryItemId` | AUTHENTICATED_USER_READ | Actor reads one actor-owned, non-Store Inventory row | ID plus actor owner predicates; generic `404` for cross-user or Store Inventory |
| PATCH | `/me/inventory/:inventoryItemId` | AUTHENTICATED_USER_WRITE | Actor edits only supported owner-safe physical/private fields | Strict DTO excludes owner, Store, printing, provenance, transaction, custody, status, and other system fields; owner predicate and existing lifecycle mutability rules apply |
| POST | `/me/inventory/:inventoryItemId/remove` | AUTHENTICATED_USER_WRITE | Actor removes eligible own current Inventory without deleting history | Owner predicate plus lifecycle and open-Listing conflict checks; sets `status = removed`, clears collection placement, and preserves the row |
...
- Step 4A introduced the first `/me/...` routes for authenticated Profile work, and Step 4B extends that pattern to owner-private Inventory. Collections remain Step 4C and bulk import remains Step 4D. A future cleanup may extend self-scoping to more domains and remove legacy actor parameters entirely; that remains API ergonomics debt, not a Step 3B security dependency.
